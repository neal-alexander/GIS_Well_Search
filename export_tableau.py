...
def main():
    ...
    indices_within_radius = tree.query_radius(
        df_surfactant[["lat_rad", "lon_rad"]].values, r=radius_rad
    )
    ...
    for i, (_, s_row) in enumerate(df_surfactant.iterrows()):
        ...
        if pd.notna(s_zone):
            valid_neighbor_labels.extend(
                neighbors_df[neighbors_df["primary_zone"] == s_zone].index.tolist()
            )
        else:
    ...
    # Add well_type
    df_full_headers["well_type"] = np.where(
        df_full_headers["uwi"].isin(surfactant_uwis), "Surfactant", "Neighbor"
    )

    # 3. Fetch EVA frac data and merge it into full headers
    print("Fetching EVA frac header data...")
    frac_query = f"""
    SELECT *
    FROM eva.eva_frac_data
    WHERE uwi IN ('{uwi_list_str}')
    """
    df_frac = pd.read_sql(frac_query, engine)
    if df_frac.empty:
        print("Warning: no EVA frac data found for the selected wells.")
    else:
        df_full_headers = df_full_headers.merge(df_frac, on="uwi", how="left")

    # 4. Fetch Production Data
    ...
    df_prod["total_vol"] = (
        df_prod["oil_volume_m3"].fillna(0)
        + df_prod["gas_volume_e3m3"].fillna(0)
        + df_prod["water_volume_m3"].fillna(0)
    )
    first_prod = (
        df_prod[df_prod["total_vol"] > 0]
        .groupby("uwi")["volume_date"]
        .min()
        .reset_index()
    )
    ...
    df_prod["effective_days"] = np.where(
        df_prod["hours_on_prod"] > 0,
        df_prod["hours_on_prod"] / 24,
        df_prod["days_in_month"],
    )
